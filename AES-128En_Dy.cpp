#include <iostream>
#include <iomanip>
#include <cstring>
#include <vector>
#include <fstream>
#include <sstream>

using namespace std;

// AES S-box and inverse S-box
unsigned char sbox[256] = {
	
};
unsigned char inv_sbox[256] = {
	
};


// Rcon values for key expansion
unsigned char rcon[256] = {
	
};
// Multiply functions for MixColumns
unsigned char mul2(unsigned char x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1B : 0x00);
}

unsigned char mul3(unsigned char x) {
    return mul2(x) ^ x;
}

unsigned char mul9(unsigned char x) {
    return mul2(mul2(mul2(x))) ^ mul2(mul2(x)) ^ x;
}

unsigned char mul11(unsigned char x) {
    return mul2(mul2(mul2(x))) ^ mul2(mul2(x)) ^ mul2(x) ^ x;
}

unsigned char mul13(unsigned char x) {
    return mul2(mul2(mul2(x))) ^ mul2(x) ^ x;
}

unsigned char mul14(unsigned char x) {
    return mul2(mul2(mul2(x))) ^ mul2(mul2(x)) ^ mul2(x) ^ x;
}

// Key Expansion functions
void KeyExpansion(unsigned char* key, unsigned char* expandedKeys) {
    unsigned char temp[4];
    int i = 0;

    // Copy the original key
    while (i < 16) {
        expandedKeys[i] = key[i];
        i++;
    }

    i = 16;
    int rconIndex = 1;

    while (i < 176) {
        // Copy the previous block
        for (int j = 0; j < 4; j++) {
            temp[j] = expandedKeys[i - 4 + j];
        }

        // Perform key schedule core
        if (i % 16 == 0) {
            // Rotate and apply S-box
            unsigned char t = temp[0];
            temp[0] = sbox[temp[1]];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
            temp[0] ^= rcon[rconIndex++];
        }

        // XOR with previous block
        for (int j = 0; j < 4; j++) {
            expandedKeys[i] = expandedKeys[i - 16] ^ temp[j];
            i++;
        }
    }
}

// AES Encryption functions
void AddRoundKey(unsigned char* state, unsigned char* roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

void SubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

void ShiftRows(unsigned char* state) {
    unsigned char temp[16];

    // Row 1
    temp[0] = state[0];
    temp[1] = state[5];
    temp[2] = state[10];
    temp[3] = state[15];

    // Row 2
    temp[4] = state[4];
    temp[5] = state[9];
    temp[6] = state[14];
    temp[7] = state[3];

    // Row 3
    temp[8] = state[8];
    temp[9] = state[13];
    temp[10] = state[2];
    temp[11] = state[7];

    // Row 4
    temp[12] = state[12];
    temp[13] = state[1];
    temp[14] = state[6];
    temp[15] = state[11];

    memcpy(state, temp, 16);
}

void MixColumns(unsigned char* state) {
    unsigned char temp[16];

    for (int i = 0; i < 4; i++) {
        temp[i * 4] = mul2(state[i * 4]) ^ mul3(state[i * 4 + 1]) ^ state[i * 4 + 2] ^ state[i * 4 + 3];
        temp[i * 4 + 1] = state[i * 4] ^ mul2(state[i * 4 + 1]) ^ mul3(state[i * 4 + 2]) ^ state[i * 4 + 3];
        temp[i * 4 + 2] = state[i * 4] ^ state[i * 4 + 1] ^ mul2(state[i * 4 + 2]) ^ mul3(state[i * 4 + 3]);
        temp[i * 4 + 3] = mul3(state[i * 4]) ^ state[i * 4 + 1] ^ state[i * 4 + 2] ^ mul2(state[i * 4 + 3]);
    }

    memcpy(state, temp, 16);
}

void EncryptBlock(unsigned char* input, unsigned char* key, unsigned char* output) {
    unsigned char state[16];
    unsigned char roundKeys[176];

    memcpy(state, input, 16);
    KeyExpansion(key, roundKeys);
    AddRoundKey(state, roundKeys);

    for (int round = 1; round < 10; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, roundKeys + round * 16);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKeys + 160);

    memcpy(output, state, 16);
}

// AES Decryption functions
void InvAddRoundKey(unsigned char* state, unsigned char* roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

void InvSubBytes(unsigned char* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = inv_sbox[state[i]];
    }
}

void InvShiftRows(unsigned char* state) {
    unsigned char temp[16];

    // Row 1
    temp[0] = state[0];
    temp[1] = state[13];
    temp[2] = state[10];
    temp[3] = state[7];

    // Row 2
    temp[4] = state[4];
    temp[5] = state[1];
    temp[6] = state[14];
    temp[7] = state[11];

    // Row 3
    temp[8] = state[8];
    temp[9] = state[5];
    temp[10] = state[2];
    temp[11] = state[15];

    // Row 4
    temp[12] = state[12];
    temp[13] = state[9];
    temp[14] = state[6];
    temp[15] = state[3];

    memcpy(state, temp, 16);
}

void InvMixColumns(unsigned char* state) {
    unsigned char temp[16];

    for (int i = 0; i < 4; i++) {
        temp[i * 4] = mul14(state[i * 4]) ^ mul11(state[i * 4 + 1]) ^ mul13(state[i * 4 + 2]) ^ mul9(state[i * 4 + 3]);
        temp[i * 4 + 1] = mul9(state[i * 4]) ^ mul14(state[i * 4 + 1]) ^ mul11(state[i * 4 + 2]) ^ mul13(state[i * 4 + 3]);
        temp[i * 4 + 2] = mul13(state[i * 4]) ^ mul9(state[i * 4 + 1]) ^ mul14(state[i * 4 + 2]) ^ mul11(state[i * 4 + 3]);
        temp[i * 4 + 3] = mul11(state[i * 4]) ^ mul13(state[i * 4 + 1]) ^ mul9(state[i * 4 + 2]) ^ mul14(state[i * 4 + 3]);
    }

    memcpy(state, temp, 16);
}

void DecryptBlock(unsigned char* input, unsigned char* key, unsigned char* output) {
    unsigned char state[16];
    unsigned char roundKeys[176];

    memcpy(state, input, 16);
    KeyExpansion(key, roundKeys);
    InvAddRoundKey(state, roundKeys + 160);

    for (int round = 9; round >= 1; round--) {
        InvShiftRows(state);
        InvSubBytes(state);
        InvAddRoundKey(state, roundKeys + round * 16);
        InvMixColumns(state);
    }

    InvShiftRows(state);
    InvSubBytes(state);
    InvAddRoundKey(state, roundKeys);

    memcpy(output, state, 16);
}

// Helper function to print in hex format
void printHex(const unsigned char* data, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        printf("%02x", data[i]);
    }
    printf("\n");
}

// Padding function
vector<unsigned char> padString(const string& input) {
    size_t paddingSize = 16 - (input.size() % 16);
    vector<unsigned char> paddedInput(input.begin(), input.end());
    paddedInput.insert(paddedInput.end(), paddingSize, paddingSize);
    return paddedInput;
}

string unpadString(const vector<unsigned char>& input) {
    size_t paddingSize = input.back();
    return string(input.begin(), input.end() - paddingSize);
}

// Main function
int main() {
    unsigned char key[16] = { /* 16 bytes key here */ };
    unsigned char input[16];
    unsigned char output[16];

    cout << "AES-128 Encryption/Decryption Tool\n";
    cout << "1. Encrypt message\n";
    cout << "2. Decrypt message\n";
    cout << "Enter option (1 or 2): ";
    int option;
    cin >> option;
    cin.ignore();  // Ignore leftover newline character

    if (option == 1) {
        // Encryption
        cout << "Enter message (normal string, will be padded to fit 16 bytes): ";
        string message;
        getline(cin, message);

        vector<unsigned char> paddedMessage = padString(message);
        vector<unsigned char> encryptedMessage(16);

        for (size_t i = 0; i < paddedMessage.size(); i += 16) {
            EncryptBlock(&paddedMessage[i], key, output);
            copy(output, output + 16, encryptedMessage.begin() + i);
        }

        ofstream outFile("message.aes", ios::binary);
        outFile.write(reinterpret_cast<char*>(encryptedMessage.data()), encryptedMessage.size());
        outFile.close();

        cout << "Encrypted message saved to message.aes\n";
        cout << "Encrypted message (hex): ";
        printHex(encryptedMessage.data(), encryptedMessage.size());
    } else if (option == 2) {
        // Decryption
        ifstream inFile("message.aes", ios::binary);
        if (!inFile) {
            cerr << "Error: Could not open message.aes\n";
            return 1;
        }
        vector<unsigned char> encryptedMessage((istreambuf_iterator<char>(inFile)), istreambuf_iterator<char>());
        inFile.close();

        vector<unsigned char> decryptedMessage(encryptedMessage.size());
        for (size_t i = 0; i < encryptedMessage.size(); i += 16) {
            DecryptBlock(&encryptedMessage[i], key, output);
            copy(output, output + 16, decryptedMessage.begin() + i);
        }

        string decryptedText = unpadString(decryptedMessage);
        cout << "Decrypted message: " << decryptedText << endl;
    } else {
        cerr << "Invalid option\n";
        return 1;
    }

    return 0;
}
